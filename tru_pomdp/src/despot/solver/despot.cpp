...
VNode* DESPOT::Trial(VNode* root, RandomStreams& streams,
	...
	do {
		...
		if (cur->IsLeaf()) {
			...
			// double root_lower_bound = cur->lower_bound();
			// double root_upper_bound = cur->upper_bound();
			// cout<<endl;
			// cout<<"cur lower bound: "<<root_lower_bound<<endl;
			// cout<<"cur upper bound: "<<root_upper_bound<<endl;
...
VNode* DESPOT::ConstructTree(vector<State*>& particles, RandomStreams& streams,
	...
	do {
		...
		double root_lower_bound = root->lower_bound();
		double root_upper_bound = root->upper_bound();
		// cout<<endl;
		// cout<<"root lower bound before backup: "<<root_lower_bound<<endl;
		// cout<<"root upper bound before backup: "<<root_upper_bound<<endl;
		start = clock();
		Backup(cur);
		if (statistics != NULL) {
		...
		// cout<<endl;
		// cout<<"root lower bound: "<<root_lower_bound<<endl;
		// cout<<"root upper bound: "<<root_upper_bound<<endl;
...
ValuedAction DESPOT::Search() {
	...
	{
		const TruPomdpTask* override_model = dynamic_cast<const TruPomdpTask*>(model_);
		// const unique_ptr<array<ActionSimple, 200>> actions;
		int legal_action_num;
		// vector<ActionSimple> all_actions;
		unordered_map<size_t, int>* action_map;
		// if (override_model != nullptr) {
		// 	vector<State*> samples = belief_->Sample(1);
		// 	if (!samples.empty()) {
		// 		const TruPomdpState* state = static_cast<const TruPomdpState*>(samples[0]);
		// 		actions = &state->legal_actions_;
		// 		legal_action_num = state->legal_action_num_;
		// 		// all_actions = *state->all_actions_ptr_;
		// 		action_map = state->action_index_map_;
		// 	}
		// }
		vector<State*> particles = belief_->Sample(1);
		const TruPomdpState* state = static_cast<const TruPomdpState*>(particles[0]);

        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dis(0, legal_action_num - 1);
        int random_index = dis(gen);

		ActionSimple action = (*state->legal_actions_)[random_index];
		...
		ACT_TYPE action_int = action_map->find(action_key)->second;

		return ValuedAction(action_int, Globals::NEG_INFTY);
	...
	cout << "[DESPOT::Search] Time for tree construction: "
		<< (get_time_second() - start) << "s" << endl;	
	...
	logi << "[DESPOT::Search] Time for freeing particles in search tree: "
		<< (get_time_second() - start) << "s" << endl;
	cout << "[DESPOT::Search] Time for freeing particles in search tree: "
		<< (get_time_second() - start) << "s" << endl;
	...
	logi << "[DESPOT::Search] Time for deleting tree: "
		<< (get_time_second() - start) << "s" << endl;
	cout << "[DESPOT::Search] Time for deleting tree: "
		<< (get_time_second() - start) << "s" << endl;
...
ValuedAction DESPOT::OptimalAction(VNode* vnode) {
	...
	for (auto & children_node : vnode->children()) {
		ACT_TYPE action = children_node->edge();
		QNode* qnode = children_node;
		if (qnode->lower_bound() > astar.value) {
			astar = ValuedAction(action, qnode->lower_bound());
		}
	}
...
void DESPOT::Backup(VNode* vnode) {
	...
	// cout << "- Backup " << vnode << " at depth " << vnode->depth() << endl;
	while (true) {
		logd << " Iter " << iter << " " << vnode << endl;
		// cout << " Iter " << iter << " " << vnode << endl;
		...
		// cout << " Updated Q-node to (" << parentq->lower_bound() << ", "
		// 	<< parentq->upper_bound() << ")" << endl;
...
void DESPOT::Expand(VNode* vnode,
	...
	// const TruPomdpTask* override_model = dynamic_cast<const TruPomdpTask*>(model);
	// vector<ACT_TYPE> actions = override_model->LegalActions(*vnode->particles()[0]);
	
	vector<ActionSimple> actions = {};
	vector<State* > particles = vnode->particles();
	ActionSimple action;
	for (int i = 0; i < particles.size(); i++) {
		TruPomdpState* state = static_cast<TruPomdpState*>(particles[i]);
		// state->UpdateActions();
		// array<ActionSimple, 200> state_actions = state->legal_actions_;
		int state_legal_action_num = state->legal_action_num_;
		for (int j = 0; j < state->legal_actions_->size(); j++) {
			action = state->legal_actions_->at(j);
	...
	// unordered_map<size_t, int> action_map = *state->action_index_map_;
	for (int i = 0; i < state->legal_action_num_; i++) {
		action = state->legal_actions_->at(i);
		size_t action_key = hash<ActionSimple>{}(action);
		ACT_TYPE action_int = state->action_index_map_->find(action_key)->second;
		QNode *qnode = new QNode(vnode, action_int);
		children.push_back(qnode);
		Expand(qnode, lower_bound, upper_bound, model, streams, history);
	}
	logd << "* Expansion complete!" << endl;
	// cout << "* Expansion complete!" << endl;
	double vnode_lower_bound = vnode->lower_bound();
	double vnode_upper_bound = vnode->upper_bound();
	// cout<<"vnode lower bound: "<< vnode_lower_bound<<endl;
	// cout<<"vnode upper bound: "<< vnode_upper_bound<<endl;
}
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
	...
	// if (step_reward > 500){
	// 	cout<<"step_reward: "<<step_reward<<endl;
	// }
	...
	for (map<OBS_TYPE, vector<State*> >::iterator it = partitions.begin();
		...
		// cout << " Creating node for obs " << obs << endl;
...
ValuedAction DESPOT::Evaluate(VNode* root, vector<State*>& particles,
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		while (!streams.Exhausted()) {
			...
			cout<<"get action int"<< action <<endl;
			assert(action != -1);
...
void DESPOT::belief(Belief* b) {
	...
	cout << "[DESPOT::belief] Start: Set initial root." << endl;
}
...
void DESPOT::BeliefUpdate(ACT_TYPE action, OBS_TYPE obs) {
	...
	cout << "[Solver::Update] Updated belief, history and root with action "
		<< action << ", observation " << obs
		<< " in " << (get_time_second() - start) << "s" << endl;
}

void DESPOT::BeliefUpdate(ACT_TYPE action, unique_ptr<SceneGraph> obs) {
	double start = get_time_second();

	// SimpleBelief *simple_belief_ = dynamic_cast<SimpleBelief*>(belief_);
	TruPomdpBelief *simple_belief_ = dynamic_cast<TruPomdpBelief*>(belief_);
	simple_belief_->Update_SceneGraph(action, std::move(obs));
	history_.Add(action, 0);

	//lower_bound_->belief(belief_);

	logi << "[Solver::Update] Updated belief, history and root with action "
		<< action << " in " << (get_time_second() - start) << "s" << endl;
	cout << "[Solver::Update] Updated belief, history and root with action "
		<< action << " in " << (get_time_second() - start) << "s" << endl;
}
